...
macOS users can download the current packaged v0.16.5 Mainnet DMGs from:

- Apple Silicon: https://github.com/CYPHES-ATP/Node/releases/download/v0.16.5/CYPHES_0.16.5_aarch64.dmg
- Intel: https://github.com/CYPHES-ATP/Node/releases/download/v0.16.5/CYPHES_0.16.5_x64.dmg

- Windows x64: https://github.com/CYPHES-ATP/Node/releases/download/v0.16.5/CYPHES_0.16.5_x64-setup.exe
...
The current source tree is v0.16.5. It keeps the
...
v0.16.5 does not require a fresh database; it is a non-mandatory
verifier-liveness upgrade with aggregate dashboard summaries, cached credit
refreshes, and coalesced network event reloads. It also refreshes stale local
claim state before running cached work units so long-running workers do not
repeat "Claim the work unit before running it" after a claim view goes stale.
Verifier-first nodes now check the durable SQLite queue for the oldest
independently verifiable submitted receipt before falling back to
frontend-hydrated campaign snapshots, so fresh or rejoining nodes can help drain
pending receipts without pressing Contribute.
