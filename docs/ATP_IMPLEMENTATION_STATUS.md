...
v0.17.0 runs mainnet as a non-mandatory settlement-liveness upgrade over the
...
binds settlement to the contribution receipt and proof hash. v0.17.0 also
requires evidence-backed structured output or one successful repair pass before
a model run counts as a full-quality proof. v0.17.0 keeps the cheap duplicate and
...
path, impact, and reproduction evidence. v0.16.2 also added explicit forward-only
...
The v0.17.0 cockpit uses an aggregate backend dashboard summary for ordinary
...
v0.17.0 adds direct settlement rescue for straggler receipts: a node with old
submitted receipts advertises the exact contribution IDs, peers answer whether
they can verify, already have a verification, need the contribution object, or
have superseding finality for the same work unit, and the sender fills the
missing dependency immediately. Live connected peers are eligible for recovery
traffic even when a stale dial-failure cooldown would block a fresh dial.
v0.17.0 keeps the libp2p infrastructure watchdog that treats silent relay and
...
of silently listening with no established connections. It also makes rendezvous
discovery dials honor the existing peer failure cooldown, preventing stale
discovery records from repeatedly consuming relay circuit budget while offline
nodes expire.
