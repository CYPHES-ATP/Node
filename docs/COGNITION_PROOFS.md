...
Every new paid contribution carries a Cognition Proof packet. v0.16.7 keeps the
...
v0.16.7 keeps the proof-quality gate before settlement. The local audit runtime
...
v0.16.7 also preserves the duplicate/superseded preflight, stays on the
...
v0.16.7 does not change the Cognition Proof wire shape. It improves how the app
...
receipts without first entering worker mode. v0.16.7 also makes that path more
reliable after sleep/wake or NAT churn by recycling silent relay/rendezvous
links, redialing infrastructure, making discovery dials honor peer failure
cooldowns, and surfacing route failures in the local labor event stream.
